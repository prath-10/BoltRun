...
from BoltRun.definitions import ABORTED_TASK
from BoltRun.tasks import *
from BoltRun.gpt import GPTAttackSuggestionGenerator
from BoltRun.utilities import is_safe_path
...
class BoltrunUpdateCheck(APIView):
	def get(self, request):
		req = self.request
		github_api = \
			'https://api.github.com/repos/yogeshojha/boltrun/releases'
		...
		current_version = ((os.environ['BOLTRUN_CURRENT_VERSION'
							])[1:] if os.environ['BOLTRUN_CURRENT_VERSION'
							][0] == 'v'
							else os.environ['BOLTRUN_CURRENT_VERSION']).replace("'", "")
