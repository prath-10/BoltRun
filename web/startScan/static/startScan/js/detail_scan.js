...
function detect_subdomain_cms(http_url, http_status){
	if (http_status == 0) {
		var message = `BoltRun has earlier identified that this subdomain did not return any HTTP status and likely the subdomain is not alive. BoltRun may not be able to detect any CMS, would you still like to continue?`;
	}
	else if (http_status != 200) {
		var message = `BoltRun has earlier identified that this subdomain has HTTP status as ${http_status} and likely that BoltRun will not detect any CMS, would you still like to continue?`;
	}
