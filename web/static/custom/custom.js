	...
	var interesting_subdomain_table = $('#interesting_subdomains').DataTable({
		"drawCallback": function(settings, start, end, max, total, pre) {
			...
			if (this.fnSettings().fnRecordsTotal() == 0) {
				...
				// <div class="card-body">
				// BoltRun could not identify any interesting subdomains. You can customize interesting subdomain keywords <a href="/scanEngine/interesting/lookup/">from here</a> and this section would be automatically updated.
				// </div>
				// </div>`);
			} else {
				// show nav bar
		...
		if (response.status) {
			...
			fetch(`/api/tools/whois/?format=json&ip_domain=${domain_name}`, {
				...
				if (response.status) {
					...
					Swal.fire({
						title: 'Oops!',
						text: `BoltRun could not fetch WHOIS records for ${domain_name}!`,
						icon: 'error'
					});
				}
			});
...
function get_domain_whois(domain_name, show_add_target_btn=false) {
	...
	fetch(url, {
		...
		if (response.status) {
			...
			Swal.fire({
				title: 'Oops!',
				text: `BoltRun could not fetch WHOIS records for ${domain_name}! ${response['message']}`,
				icon: 'error'
			});
		}
	});
	...
	fetch('/api/fetch/most_vulnerable/?format=json', {
		...
		if (response.status) {
			...
			$('#most_vulnerable_target_div').append(`
				...
				Once the vulnerability scan is performed, BoltRun will identify the most vulnerable targets.</div>
			`);
		}
	});
}
	...
	fetch('/api/fetch/most_common_vulnerability/?format=json', {
		...
		if (response.status) {
			...
			$('#most_common_vuln_div').append(`
				...
				Once the vulnerability scan is performed, BoltRun will identify the Most Common Vulnerabilities.</div>
			`);
		}
	});
}
