...
function cms_detector(){
	...
	$('#modal-content').append(`
		...
		<small class="mb-3 float-end text-muted">(BoltRun uses <a href="https://github.com/Tuhinshubhra/CMSeeK" target="_blank">CMSeeK</a> to detect CMS.)</span>
...
function cms_detector_api_call(url){
	...
	Swal.fire({
		title: `Detecting CMS`,
		text: `BoltRun is detecting CMS on ${url} and this may take a while. Please wait...`,
...
function toolbox_waf_detector(){
	...
	$('#modal-content').append(`
		...
		<small class="mb-3 float-end text-muted">(BoltRun uses <a href="https://github.com/EnableSecurity/wafw00f" target="_blank">wafw00f</a> to detect WAF.)</span>
		<div class="mt-3 mb-3 text-center">
...
function waf_detector_api_call(url){
	...
	Swal.fire({
		title: `Detecting WAF`,
		text: `BoltRun is detecting WAF on ${url} and this may take a while. Please wait...`,
		allowOutsideClick: false
	});
	swal.showLoading();
	fetch(api_url, {
