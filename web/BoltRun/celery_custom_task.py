...
from BoltRun.common_func import (fmt_traceback, get_output_file_name,
...
class BoltrunTask(Task):
	...
	def __call__(self, *args, **kwargs):
		...
		self.results_dir = ctx.get('results_dir', BOLTRUN_RESULTS)
		...
		if BOLTRUN_CACHE_ENABLED:
			...
			if result and result != b'null':
				...
				if BOLTRUN_RECORD_ENABLED and self.track:
		...
		if BOLTRUN_CACHE_ENABLED and self.status == SUCCESS_TASK and result:
	...
	def notify(self, name=None, severity=None, fields={}, add_meta_info=True):
		...
		from BoltRun.tasks import send_task_notif
