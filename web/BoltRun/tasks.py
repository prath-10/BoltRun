...
from BoltRun.celery import app
from BoltRun.gpt import GPTVulnerabilityReportGenerator
from BoltRun.celery_custom_task import BoltrunTask
from BoltRun.common_func import *
from BoltRun.definitions import *
from BoltRun.settings import *
from BoltRun.gpt import *
from BoltRun.utilities import *
...
def initiate_subscan(
	...
	if not method:
		logger.warning(f'Task {scan_type} is not supported by BoltRun. Skipping')
		return
...
#------------------------- #
# Tracked BoltRun tasks    #
#--------------------------#

@app.task(name='subdomain_discovery', queue='main_scan_queue', base=BoltrunTask, bind=True)
def subdomain_discovery(
	...
	for tool in tools:
		...
		if tool in default_subdomain_tools:
			...
			logger.warning(
				f'Subdomain discovery tool "{tool}" is not supported by BoltRun. Skipping.')
			continue
...
@app.task(name='osint', queue='main_scan_queue', base=BoltrunTask, bind=True)
...
@app.task(name='screenshot', queue='main_scan_queue', base=BoltrunTask, bind=True)
...
@app.task(name='port_scan', queue='main_scan_queue', base=BoltrunTask, bind=True)
...
@app.task(name='nmap', queue='main_scan_queue', base=BoltrunTask, bind=True)
def nmap(
		self,
		cmd=None,
		ports=[],
...
@app.task(name='waf_detection', queue='main_scan_queue', base=BoltrunTask, bind=True)
...
@app.task(name='dir_file_fuzz', queue='main_scan_queue', base=BoltrunTask, bind=True)
...
@app.task(name='fetch_url', queue='main_scan_queue', base=BoltrunTask, bind=True)
...
@app.task(name='vulnerability_scan', queue='main_scan_queue', bind=True, base=BoltrunTask)
...
@app.task(name='nuclei_individual_severity_module', queue='main_scan_queue', base=BoltrunTask, bind=True)
...
@app.task(name='nuclei_scan', queue='main_scan_queue', base=BoltrunTask, bind=True)
...
@app.task(name='dalfox_xss_scan', queue='main_scan_queue', base=BoltrunTask, bind=True)
...
@app.task(name='crlfuzz_scan', queue='main_scan_queue', base=BoltrunTask, bind=True)
...
@app.task(name='s3scanner', queue='main_scan_queue', base=BoltrunTask, bind=True)
...
@app.task(name='http_crawl', queue='main_scan_queue', base=BoltrunTask, bind=True)
def http_crawl(
		self,
		urls=[],
		method=None,
...
def send_file_to_discord(file_path, title=None):
	...
	webhook = DiscordWebhook(
		...
		username=title or "BoltRun Discord Plugin"
	)
...
def save_imported_subdomains(subdomains, ctx={}):
	...
	domain = Domain.objects.get(pk=domain_id)
	results_dir = ctx.get('results_dir', BOLTRUN_RESULTS)
