...
from BoltRun.tasks import (run_command, send_discord_message, send_slack_message, send_telegram_message)
...
def boltrun_settings(request, slug):
    context = {}
    ...
    context['settings_nav_active'] = 'active'
    context['boltrun_settings_li'] = 'active'
    context['settings_ul_show'] = 'show'

    return render(request, 'scanEngine/settings/boltrun.html', context)
...
def notification_settings(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            send_slack_message('*BoltRun*\nCongratulations! your notification services are working.')
            send_telegram_message('*BoltRun*\nCongratulations! your notification services are working.')
            send_discord_message('**BoltRun**\nCongratulations! your notification services are working.')
            messages.add_message(
                request,
                messages.INFO,
