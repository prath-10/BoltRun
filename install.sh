...
echo " "
tput setaf 4;
echo "Installing BoltRun and it's dependencies"

echo " "
if [ "$EUID" -ne 0 ]
  then
  tput setaf 1; echo "Error installing BoltRun, Please run this script as root!"
...
tput setaf 4;
echo "#########################################################################"
echo "Installing BoltRun"
echo "#########################################################################"
make certs && make build && make up && tput setaf 2 && echo "BoltRun is installed!!!" && failed=0 || failed=1
...
if [ "${failed}" -eq 0 ]; then
  ...
  tput setaf 2 && printf "\n%s\n" "Thank you for installing BoltRun, happy recon!!"
else
  tput setaf 1 && printf "\n%s\n" "BoltRun installation failed!!"
fi
